...
@Composable
fun OrderHistorySuccessDetailScreenRoute(viewModel: OrderHistorySuccessDetailViewModel = viewModel()) {
    OrderHistorySuccessDetailScreen(state = viewModel.state) { viewModel.on_event(OrderHistorySuccessDetailScreenEvent.RetryRequested) }
}
...
fun OrderHistorySuccessDetailScreen(
    ...
    ScreenScaffold(screenName = "order_history_success_detail_screen", state = state, onRetry = onRetry)
}
